...
For scenario visualization: https://www.youtube.com/watch?v=yn299EEBTVI
...
The goal is to find the earliest time when the frog can jump to the other
 ...
 from 1 to X (that is, we want to find the earliest moment when all the positions from 1 to X are covered by leaves). You may assume that the speed of the current in the river is negligibly small, i.e. the leaves do not change their positions once they fall in the river.
...
FOR SIMPLICITY:
AT WHAT TIME WERE ALL POSITIONS ARE FILLED?
*/
function solution($goal, $leaves) {
    // write your code in PHP7.0
    $leafPosition = [];
    foreach($leaves as $time => $leaf){
        $leafPosition[$leaf] = $leaf;
        // var_dump($leafPosition);
        if(count($leafPosition) == $goal){        
            return $time;
        }    
    }
    return -1; //returning -1 for input e.g. solution(5,[3]).
}

// $leaves = [3];
// $goal = 5;
// $time = solution($goal, $leaves);
